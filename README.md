...
### 方式一：直接双击打开（预编译版本，适合快速预览 ⚡）
我们已经将项目打包编译好的静态文件放到了仓库中。你**不需要安装任何环境（如 Node.js 或 Git）**，即可直接体验：
1. 下载或克隆本仓库到本地。
2. 进入根目录下的 **`dist/`** 文件夹。
3. 双击直接用任意浏览器打开 **[index.html](dist/index.html)** 即可使用主要预测功能！
   *(注：我们已将 Vite 静态资源配置为相对路径 `./`，因此无需启动本地 Web 服务即可本地直接双击预览。)*

> [!WARNING]
> **自定义大模型（LLM）API 的跨域（CORS）说明**：
> - **Google Gemini API**：原生支持浏览器跨域直连，即使双击 HTML 直接打开，配置 Key 后也可正常使用。
> - **DeepSeek 等其他第三方 API / 高校私有 API（如上科大 API）**：由于其服务端限制了浏览器的跨域请求，直接双击 HTML 打开使用时会被浏览器 CORS 拦截。**若需正常使用这部分自定义大模型功能，请使用下方的【方式二】启动本地开发或预览服务。**
...
#### 步骤 2：启动本地开发服务器
运行 Vite 本地开发命令（内置 API CORS 跨域代理）：
```bash
npm run dev
```
打开浏览器访问：
...
npm run build
```
这会在根目录下生成 `dist/` 文件夹，里面包含了经过压缩优化的高性能单页应用文件（HTML/CSS/JS）。

#### 步骤 4：本地预览生产包并启用代理
如果您想在本地预览 `dist/` 的打包效果，并确保自定义大模型 API 跨域转发正常工作，请运行：
```bash
npm run preview
```
打开浏览器访问：
👉 **[http://localhost:3000](http://localhost:3000)**
*(该命令会启用 Vite 生产预览服务器，加载 `vite.config.ts` 中的代理配置，完美绕过跨域限制。)*
